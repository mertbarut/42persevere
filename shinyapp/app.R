...
network_data <- data_complete %>% filter(evals_coded == 1, campus %in% shown_campuses, month %in% shown_months, Student %in% show_studentship)
...
ui <- dashboardPage(
	...
	sidebar <- dashboardSidebar(
		...
		checkboxGroupInput("shown_projects", "Select projects", choices = c(
			"Shell 00" = "s00", "Shell 01" = "s01",
	    	"C 00" = "c00", "C 01" = "c01", "C 02" = "c02",
	    	"C 03" = "c03", "C 04" = "c04", "C 05" = "c05",
	    	"C 06" = "c06", "C 07" = "c07", "C 08" = "c08",
	    	"C 09" = "c09", "C 10" = "c10", "C 11" = "c11",
	    	"C 12" = "c12", "C 13" = "c13"), selected = 1),
...
server <- function(input, output, session) {
	output$network <- renderVisNetwork(
	{
    	...
    	isolate({
			...
			validate(need((input$login != "" && input$login %in% network_data$login), 'Intra login was not found\n(This is likely because I did not enter it into the data yet)'))
			validate(need((input$shown_projects != ""), 'No projects are selected'))
			...
			nodes <- nodes %>% mutate(id = row_number(), shape = "dot", color = "#DADCDE", size = 5, font.size = 5, font.color = "#DADCDE") %>% relocate(id, .before = id_user) %>% rename(label = id_user)		
			nodes[nodes$label == input$login, "shape"] <- "dot"
			nodes[nodes$label == input$login, "size"] <- 10
			nodes[nodes$label == input$login, "color"] <- "#FF9A00"
			nodes[nodes$label == input$login, "font.size"] <- 20
			...
			nodes[nodes$label %in% evaluate_to$dest, "size"] <- 10
			nodes[nodes$label %in% evaluate_to$dest, "font.size"] <- 20
			nodes[nodes$label %in% evaluate_to$dest, "color"] <- "#FF9A00"
			nodes[nodes$label %in% evaluate_to$dest, "font.color"] <- "#343434"
			nodes[nodes$label %in% evaluate_from$src, "size"] <- 10
			nodes[nodes$label %in% evaluate_from$src, "font.size"] <- 20
			...
			visNetwork(nodes, edges) %>%
			    ...
			    visInteraction(hover = TRUE) %>%
					visIgraphLayout(layout = "layout_with_fr", randomSeed = 42, type = "square") %>%
